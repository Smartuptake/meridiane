...
# LXGW WenKai (Kaiti-Schnitt) wird auf genau die Zeichen verkleinert, die im Projekt vorkommen.
...
import glob, re, os
chars = set()
dateien = glob.glob("data/*.js") + glob.glob("js/*.js") + glob.glob("*.html") + glob.glob("uebungen/*.html")
for f in dateien:
...
curl -s -A "$UA" -G "https://fonts.googleapis.com/css2" \
  --data-urlencode "family=LXGW WenKai TC" --data-urlencode "text=$HANZI" -o fonts/_kaiti.css
...
out = ["/* Healthlane Academy - selbst gehostete Schriften.",
       ...
       "   Cormorant Garamond, Lato, LXGW WenKai: SIL Open Font License. */", ""]
def hole(url, ziel):
...
src = open("fonts/_kaiti.css", encoding="utf-8").read()
block = re.search(r"@font-face\s*\{.*?\}", src, re.S).group(0)
m = re.search(r"url\((https://fonts\.gstatic\.com/[^)]*)\)", block)
hole(m.group(1), "fonts/kaiti-subset.woff2")
block = block.replace(m.group(1), "kaiti-subset.woff2")
